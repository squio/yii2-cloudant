Yii Framework 2 cloudant extension Change Log
==================================================
...
- Enh: Added `ActiveFixture` class for testing fixture support for cloudant (cebe, viilveer)


2.0.1 December 07, 2014
-----------------------

- Bug #5662: Cloudant AR updateCounters() now uses explicitly `groovy` script for updating making it compatible with ES >1.3.0 (cebe)
...
- Bug #4187: Cloudant dynamic scripting is disabled in 1.2.0, so do not use it in query builder (cebe)
...
- Chg: Requirement changes: Yii now requires cloudant version 1.0 or higher (cebe)
...
- Enh #1313: made index and type available in `ActiveRecord::instantiate()` to allow creating records based on cloudant type when doing cross index/type search (cebe)
- Enh #1382: Added a debug toolbar panel for cloudant (cebe)
- Enh #1765: Added support for primary key path mapping, pk can now be part of the attributes when mapping is defined (cebe)
- Enh #2002: Added filterWhere() method to yii\cloudant\Query to allow easy addition of search filter conditions by ignoring empty search fields (samdark, cebe)
