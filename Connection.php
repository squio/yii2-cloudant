...
class Connection extends Component
{
     ...
     * @see http://www.cloudant.org/guide/en/cloudant/reference/current/cluster-nodes-info.html#cluster-nodes-info
     */
    public $nodes = [
     ...
     */
    public $activeNode;
    // TODO http://www.cloudant.org/guide/en/cloudant/client/php-api/current/_configuration.html#_example_configuring_http_basic_auth
    public $auth = [];
    /**
     * @var float timeout to use for connecting to an cloudant node.
    ...
    /**
     * @var float timeout to use when reading the response from an cloudant node.
    ...
    public function init()
    {
        foreach ($this->nodes as $node) {
            if (!isset($node['http_address'])) {
                throw new InvalidConfigException('Cloudant node needs at least a http_address configured.');
            }
        }
    }
    ...
    public function open()
    {
        ...
        if (empty($this->nodes)) {
            throw new InvalidConfigException('cloudant needs at least one node to operate.');
        }
        ...
        Yii::trace('Opening connection to cloudant. Nodes in cluster: ' . count($this->nodes)
    ...
    public function close()
    {
        ...
        Yii::trace('Closing connection to cloudant. Active node was: '
    ...
    protected function httpRequest($method, $url, $requestBody = null, $raw = false)
    {
        ...
        $options = [
            ...
            // https://secure.php.net/manual/en/function.curl-setopt.php#82418
            CURLOPT_HTTPHEADER     => ['Expect:', "Content-Type: application/json"],

            // CURLOPT_REFERER => 'http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . "{$_SERVER['HTTP_HOST']}/{$_SERVER['REQUEST_URI']}",
        ...
        if (curl_exec($curl) === false) {
            throw new Exception('Cloudant request failed: ' . curl_errno($curl) . ' - ' . curl_error($curl), [
        ...
        if ($responseCode >= 200 && $responseCode < 300) {
            if ($method == 'HEAD') {
                ...
                if (isset($headers['content-length']) && ($len = mb_strlen($body, '8bit')) < $headers['content-length']) {
                    throw new Exception("Incomplete data received from server: $len < {$headers['content-length']}", [
                        'requestMethod' => $method,
                        'requestUrl' => $url,
                        'requestBody' => $requestBody,
                        // 'requestOptions' => print_r($options, true),
...
//                }
                throw new Exception('Unsupported data received from cloudant: ' . $headers['content-type'], [
                    'requestMethod' => $method,
                    'requestUrl' => $url,
                    'requestBody' => $requestBody,
                    // 'requestOptions' => print_r($options, true),
