...
/**
 * Query represents a query to the search API of cloudant.
 *
 ...
 * $rows = $command->search(); // this way you get the raw output of cloudant.
 * ~~~
 *
 ...
 *
 * NOTE: cloudant limits the number of records returned to 10 records by default.
...
class Query extends Component implements QueryInterface
{
     ...
     *
     * [always returned as arrays]: http://www.cloudant.org/guide/en/cloudant/reference/1.x/_return_values.html#_return_values
     * [script field]: http://www.cloudant.org/guide/en/cloudant/reference/current/search-request-script-fields.html
     *
     * @see http://www.cloudant.org/guide/en/cloudant/reference/current/search-request-fields.html#search-request-fields
     * @see http://www.cloudant.org/guide/en/cloudant/reference/current/search-request-script-fields.html
     * @see fields()
     * @see source
     */
    public $fields;
     ...
     * of a record will be available in the result.
     * @see http://www.cloudant.org/guide/en/cloudant/reference/current/search-request-source-filtering.html
     * @see source()
     * @see fields
     */
    public $source;
     ...
     * @see timeout()
     * @see http://www.cloudant.org/guide/en/cloudant/reference/current/search-request-body.html#_parameters_3
     */
    public $timeout;
    /**
     * @var array|string The query part of this search query. This is an array or json string that follows the format of
     * the cloudant [Query DSL](http://www.cloudant.org/guide/en/cloudant/reference/current/query-dsl.html).
     */
    public $query;
    /**
     ...
     * @see http://www.cloudant.org/guide/en/cloudant/reference/1.x/search-aggregations.html
     */
    public $aggregations = [];
    /**
    ...
    {
        parent::init();
        // setting the default limit according to cloudant defaults
        // http://www.cloudant.org/guide/en/cloudant/reference/current/search-request-body.html#_parameters_3
        if ($this->limit === null) {
            $this->limit = 10;
        }
    }
    ...
    {
        if ($db === null) {
            $db = Yii::$app->get('cloudant');
        }

        $commandConfig = $db->get()->build($this);
     ...
     *
     *  - [routing](http://www.cloudant.org/guide/en/cloudant/reference/current/search.html#search-routing)
     *  - [search_type](http://www.cloudant.org/guide/en/cloudant/reference/current/search-request-search-type.html)
     *
    ...
    public function search($db = null, $options = [])
    {
        ...
        if (!empty($result['docs']) && $this->indexBy !== null) {
            $rows = [];
            foreach ($result['docs'] as $key => $row) {
                if (is_string($this->indexBy)) {
     ...
     *
     * Everything except query and selector will be ignored.
     *
     * @param Connection $db the database connection used to execute the query.
     * If this parameter is not given, the `cloudant` application component will be used.
    ...
    /**
     * Sets the selector part of this search query.
     ...
     * @return static the query object itself
     * @see http://www.cloudant.org/guide/en/cloudant/reference/current/search-search.html#search-multi-index-type
     */
     ...
     * @return static the query object itself
     * @see http://www.cloudant.org/guide/en/cloudant/reference/current/search-request-fields.html
     */
